...
###################################################################   CONFIG
        ...
        for conf in git-prompt.conf /etc/git-prompt.conf ~/.git-prompt.conf ~/.config/git-prompt.conf;  do
                if [[ -r $conf ]]  ;  then 
                        . $conf
                        break;
                fi
        ...
        #####  default PS1 - combined 1-line prompt
                vcs_status=""
        set_PS1() {
                local vcs_status="${vcs_info:+$vcs_color($vcs_info $file_list$vcs_color)}"
	        local cmd_line_bg="\[`tput setab 234; tput el;`\]"
                #PS1="$rc ${vcs_status} $color_who_where$dir_color\w$prompt_char $colors_reset"
                PS1="$cmd_line_bg$rc${vcs_status} $color_who_where$dir_color\W$prompt_char $colors_reset"
        }
	...
	root_prompt_char=${root_prompt_char:-'>'}
        ...
        max_file_list_length=${max_file_list_length:-150}
...
#####################################################################  post config
        ...
        ####################################################################  MARKERS
        if [[ "$LC_CTYPE $LC_ALL" =~ "[Uu][Tt][Ff]" && $TERM != "linux" ]];  then
                elipses_marker="…"
        else
                elipses_marker="..."
        fi
...
set_shell_label() {
                        # TODO http://ubuntuforums.org/showthread.php?t=448614

        xterm_label() {
                local args="$*"
                echo  -n ']2;'"${args:0:$(($COLUMNS-10))}"'' ;    # FIXME: replace hardcodes with terminfo codes
        }
...
##################################################################### HOST
        ...
        if [[ -n $host ]] ;  then 
                ...
                if [[ -z $host_color && -x /usr/bin/cksum ]] ;  then
                        cksum_color_no=`echo $host | cksum  | awk '{print $1%6}'`
                        host_color_list=(green yellow blue magenta cyan white)              # FIXME:  bw,  color-256
                        host_color=${host_color_list[cksum_color_no]}
                fi
...
parse_git_status() {
        ...
        # TODO add status rebaseing: http://trevmex.com/post/27911723383/adding-git-info-to-your-bash-prompt
